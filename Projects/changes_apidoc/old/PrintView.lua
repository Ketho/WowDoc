...
function m:PrintView(changes, isWiki)
	for _, apiType in pairs(p.apiType_order) do
		...
		for _, name in pairs(table_sort.ByKey(t[1])) do
			local s = FormatEnum(t[1][name]) or name
			print("+ "..s)
		end
		for _, name in pairs(table_sort.ByKey(t[2])) do
			print("- "..name)
		end
		for _, name in pairs(table_sort.ByKey(t[3])) do
			local v = t[3][name]
...
function m:PrintParamChanges(param, a, b, isWiki)
	...
	elseif a and b then
		...
		for _, tbl in pairs(table_sort.ByKeyValue(t_b, SortIndex)) do
		...
		for _, tbl in pairs(table_sort.ByKeyValue(t_a, SortIndex)) do
